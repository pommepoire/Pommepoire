...
export default function App() {
  ...
  useEffect(()=>{
    const unsub=onAuthStateChanged(auth,async u=>{
      setUser(u);
      if(u){const userDoc=await getDoc(doc(db,"users",u.uid));if(userDoc.exists()&&userDoc.data().tripId)setTripId(userDoc.data().tripId);}
      setLoading(false);
    });
    return unsub;
  },[]);
  ...
  useEffect(()=>{
    if(!tripId)return;
    return onSnapshot(doc(db,"trips",tripId),snap=>{if(snap.exists())setTrip({id:snap.id,...snap.data()});});
  },[tripId]);
  ...
  const dm=darkMode;
  const C={bg:dm?"#111":"#fff",bg2:dm?"#1c1c1e":"#f8f9fa",text:dm?"#fff":"#111",text2:dm?"#aaa":"#666",border:dm?"#333":"#e8e8e8"};

  const daysUntil=()=>{if(!trip?.dateStart)return null;const diff=Math.ceil((new Date(trip.dateStart)-new Date())/86400000);return diff>0?diff:null;};

  async function addReservation(data){
    await addDoc(collection(db,"trips",tripId,"reservations"),{...data,createdBy:user.uid,createdByName:user.displayName||"Moi",createdAt:serverTimestamp()});
    await addDoc(collection(db,"trips",tripId,"activity"),{text:`${user.displayName||"Un voyageur"} a ajouté : ${data.name}`,icon:{vol:"✈️",hotel:"🏨",transport:"🚌",activite:"🎭",restaurant:"🍽️"}[data.type]||"📌",createdAt:serverTimestamp(),uid:user.uid});
  }
  async function updateReservation(id,data){await updateDoc(doc(db,"trips",tripId,"reservations",id),data);}
  async function deleteReservation(id){await deleteDoc(doc(db,"trips",tripId,"reservations",id));}
  function handleAuth(u,tId){setUser(u);setTripId(tId);}
  async function handleSignOut(){await signOut(auth);setUser(null);setTripId(null);setTrip(null);setReservations([]);}
  async function switchTrip(tId){await updateDoc(doc(db,"users",user.uid),{tripId:tId});setTripId(tId);setShowTripSelector(false);}

  if(loading)return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",background:"#1a6bb5"}}><div style={{fontSize:48,marginBottom:12}}>✈️</div><div style={{fontSize:16,color:"white",opacity:0.9}}>Chargement...</div></div>);
  if(!user||!tripId)return(<div style={{maxWidth:430,margin:"0 auto",height:"100dvh",overflow:"hidden"}}><AuthPage onAuth={handleAuth}/></div>);
      ...
      <div style={{height:"100%",display:"flex",flexDirection:"column",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif",position:"relative"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{cursor:"pointer",flex:1,minWidth:0}} onClick={()=>setShowTripSelector(true)}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6,marginTop:3,flexWrap:"nowrap"}}>
                ...
                {countdown&&<div style={{flexShrink:0,background:"rgba(255,255,255,0.25)",borderRadius:99,padding:"2px 10px",fontSize:12,fontWeight:700,color:"white",whiteSpace:"nowrap"}}>J-{countdown}</div>}
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8,marginLeft:12,flexShrink:0}}>
              ...
              <div style={{display:"flex"}}>
                {memberInitials.slice(0,3).map((m,i)=>(
                  <button key={m.uid} onClick={()=>setShowShare(true)} style={{width:30,height:30,borderRadius:"50%",border:"2px solid rgba(255,255,255,0.5)",color:"white",fontSize:11,fontWeight:700,cursor:"pointer",marginLeft:i===0?0:-6,zIndex:3-i,background:"rgba(255,255,255,0.25)"}}>{m.initial}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        <div style={{display:"flex",borderBottom:`1px solid ${C.border}`,background:C.bg,flexShrink:0}}>
          {["agenda","reservations","budget"].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:"11px 8px",fontSize:12,color:tab===t?"#1a6bb5":C.text2,border:"none",background:"none",borderBottom:tab===t?"2px solid #1a6bb5":"2px solid transparent",cursor:"pointer",fontWeight:tab===t?700:400}}>
        ...
        {/* FAB */}
        <button onClick={()=>setShowTools(true)} style={{position:"absolute",bottom:"max(env(safe-area-inset-bottom),16px)",right:20,width:50,height:50,borderRadius:"50%",background:"#1a6bb5",border:"none",cursor:"pointer",fontSize:24,boxShadow:"0 4px 12px rgba(26,107,181,0.4)",zIndex:10,display:"flex",alignItems:"center",justifyContent:"center"}}>🧰</button>

        {showForm&&<ReservationForm onClose={()=>{setShowForm(false);setEditingReservation(null);}} onSave={addReservation} onUpdate={updateReservation} trip={trip} editing={editingReservation} C={C} tripId={tripId} members={trip?.memberNames}/>}
