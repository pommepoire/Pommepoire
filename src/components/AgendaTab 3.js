...
export default function AgendaTab({reservations,trip,onAdd,onEdit,currentUser,C}){
  ...
  const dayRes=activeDay?reservations.filter(r=>r.dateStart&&r.dateEnd&&normalizeDate(r.dateStart)<=normalizeDate(activeDay)&&normalizeDate(r.dateEnd)>=normalizeDate(activeDay)):[];
  return(
    <div>
      <div style={{display:"flex",background:C.bg2,borderRadius:10,padding:3,marginBottom:14}}>
        {["semaine","mois","voyage"].map(v=>(
          <button key={v} onClick={()=>setView(v)} style={{flex:1,padding:7,textAlign:"center",fontSize:11,borderRadius:8,border:"none",background:view===v?C.bg:"none",color:view===v?"#1a6bb5":C.text2,cursor:"pointer",fontWeight:view===v?700:400}}>
      ...
      {view==="semaine"&&(<>
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:10,marginBottom:12}}>
          {tripDays.slice(0,14).map(d=>{
            const hasRes=reservations.some(r=>normalizeDate(r.dateStart)<=normalizeDate(d.iso)&&normalizeDate(r.dateEnd)>=normalizeDate(d.iso));
            const active=activeDay===d.iso;
            return(
              <div key={d.iso} onClick={()=>setSelectedDay(d.iso)} style={{flexShrink:0,width:44,textAlign:"center",padding:"8px 0",borderRadius:10,cursor:"pointer",border:`0.5px solid ${active?"#1a6bb5":C.border}`,background:active?"#1a6bb5":"transparent"}}>
        ...
        {dayRes.length===0?<div style={{textAlign:"center",padding:"24px 0",color:C.text2,fontSize:13}}>Journée libre — ajoutez une activité</div>:
          dayRes.map((r,i)=>(
            <div key={r.id} style={{display:"flex",gap:10,marginBottom:10}}>
              <div style={{width:38,fontSize:11,color:C.text2,textAlign:"right",paddingTop:3,flexShrink:0}}>{r.timeStart||"--"}</div>
              ...
              <div style={{flex:1,background:C.bg2,borderRadius:10,padding:"9px 11px",borderLeft:`3px solid ${COLORS[r.type]||"#888"}`,cursor:"pointer"}} onClick={()=>onEdit(r)}>
                ...
                {r.price&&(normalizeDate(r.dateStart)===normalizeDate(activeDay)||normalizeDate(r.dateStart)===normalizeDate(r.dateEnd))&&<div style={{fontSize:11,color:C.text2,marginTop:2}}>💶 {r.price} €</div>}
      ...
      {view==="voyage"&&(
        <div>
          ...
          {tripDays.map(d=>{
            const dr=reservations.filter(r=>normalizeDate(r.dateStart)<=normalizeDate(d.iso)&&normalizeDate(r.dateEnd)>=normalizeDate(d.iso));
                ...
                <div style={{flex:1}}>
                  {dr.map(r=>(
                    <div key={r.id} style={{borderRadius:8,padding:"8px 10px",marginBottom:4,background:BG[r.type]||"#f0f0f0",cursor:"pointer"}} onClick={()=>onEdit(r)}>
                      ...
                      {(r.timeStart||( r.price&&(normalizeDate(r.dateStart)===normalizeDate(d.iso)||normalizeDate(r.dateStart)===normalizeDate(r.dateEnd))))&&(
                      <div style={{fontSize:11,color:COLORS[r.type],opacity:0.8,marginTop:1}}>
                        {r.timeStart||""}{r.timeStart&&r.price&&(normalizeDate(r.dateStart)===normalizeDate(d.iso)||normalizeDate(r.dateStart)===normalizeDate(r.dateEnd))?" · ":""}{r.price&&(normalizeDate(r.dateStart)===normalizeDate(d.iso)||normalizeDate(r.dateStart)===normalizeDate(r.dateEnd))?`${r.price}€`:""}
                      </div>
                    )}
                    </div>
                  ))}
      ...
      <button onClick={onAdd} style={{width:"100%",padding:11,border:`1.5px dashed ${C.border}`,borderRadius:12,background:"none",color:C.text2,fontSize:14,cursor:"pointer",marginTop:8}}>+ Ajouter une réservation ou activité</button>
    </div>
  );
}
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <div><button onClick={()=>setOffset(o=>o-1)} style={{border:"none",background:"none",cursor:"pointer",color:C.text2,fontSize:20,padding:"0 6px"}}>‹</button><button onClick={()=>setOffset(o=>o+1)} style={{border:"none",background:"none",cursor:"pointer",color:C.text2,fontSize:20,padding:"0 6px"}}>›</button></div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
        ...
        {Array.from({length:daysInMonth}).map((_,i)=>{
          ...
          return(
            <div key={dayNum} onClick={()=>dayRes.length>0&&onDayClick(iso)} style={{minHeight:46,padding:4,borderRadius:6,background:inTrip?C.bg2:"transparent",cursor:dayRes.length>0?"pointer":"default"}}>
              ...
              {dayRes.slice(0,2).map((r,j)=><div key={j} style={{width:5,height:5,borderRadius:"50%",background:COLORS[r.type]||"#888",margin:"1px auto"}}></div>)}
            </div>
          );
        })}
      </div>
...
function getTripDays(trip){
  ...
  while(cur<=end){days.push({iso:cur.toISOString().split("T")[0],num:cur.getDate(),name:dayNames[cur.getDay()],monthShort:months[cur.getMonth()]});cur.setDate(cur.getDate()+1);}
  return days;
}
function typeIcon(type){return{vol:"✈️",hotel:"🏨",transport:"🚌",activite:"🎭",restaurant:"🍽️",autre:"📌"}[type]||"📌";}
