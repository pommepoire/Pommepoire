...
export default function BudgetTab({ reservations, trip, tripId, C }) {
  ...
  CATEGORIES.forEach(c=>{
    spent[c.key] = reservations.filter(r=>c.types.includes(r.type)).reduce((sum,r)=>sum+(parseFloat(r.price)||0),0);
  });
  const totalBudget = Object.values(budgets).reduce((a,b)=>a+parseFloat(b||0),0);
  const totalSpent = Object.values(spent).reduce((a,b)=>a+b,0);

  // Par personne basé sur paidBy et splitAmounts
  const byPerson = {};
  reservations.forEach(r=>{
    const price = parseFloat(r.price)||0;
    if (r.paymentStatus==="Payé en totalité"&&r.paidBy) {
      byPerson[r.paidBy]=(byPerson[r.paidBy]||0)+price;
    } else if (r.paymentStatus==="Paiement divisé"&&r.splitAmounts) {
      Object.entries(r.splitAmounts).forEach(([name,amt])=>{
        byPerson[name]=(byPerson[name]||0)+(parseFloat(amt)||0);
      });
    } else {
      const name = r.createdByName||"Inconnu";
      byPerson[name]=(byPerson[name]||0)+price;
    }
  });
  ...
  async function saveBudget(key,val) {
    await updateDoc(doc(db,"trips",tripId),{budgets:{...budgets,[key]:parseFloat(val)||0}});
  ...
  return (
    <div>
      <div style={{background:C.bg2,borderRadius:12,padding:14,marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontSize:13,color:C.text2}}>Total dépensé</div><div style={{fontSize:11,color:C.text2}}>sur {totalBudget.toFixed(0)} € de budget</div></div>
        <div style={{fontSize:22,fontWeight:700,color:"#1a6bb5"}}>{totalSpent.toFixed(0)} €</div>
      </div>

      <div style={{display:"flex",gap:8,marginBottom:14}}>
        <button onClick={()=>setShowPerPerson(false)} style={{flex:1,padding:"8px",borderRadius:10,border:`1px solid ${!showPerPerson?"#1a6bb5":C.border}`,background:!showPerPerson?"#e6f1fb":"none",color:!showPerPerson?"#1a6bb5":C.text2,fontSize:13,fontWeight:!showPerPerson?700:400,cursor:"pointer"}}>Par catégorie</button>
        <button onClick={()=>setShowPerPerson(true)} style={{flex:1,padding:"8px",borderRadius:10,border:`1px solid ${showPerPerson?"#1a6bb5":C.border}`,background:showPerPerson?"#e6f1fb":"none",color:showPerPerson?"#1a6bb5":C.text2,fontSize:13,fontWeight:showPerPerson?700:400,cursor:"pointer"}}>Par personne</button>
      </div>

      {!showPerPerson?(
        <>
          {CATEGORIES.map(c=>{
            const s=spent[c.key]||0, b=parseFloat(budgets[c.key])||0;
            ...
            return(
              <div key={c.key} style={{marginBottom:10,borderRadius:12,border:`0.5px solid ${C.border}`,overflow:"hidden"}}>
                <div onClick={()=>setExpandedCat(isExp?null:c.key)} style={{padding:"12px 14px",cursor:"pointer",background:C.bg}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                    <span style={{fontSize:13,fontWeight:600,color:C.text}}>{c.icon} {c.label}</span>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      {editingBudget===c.key?(
                        <div style={{display:"flex",gap:6}} onClick={e=>e.stopPropagation()}>
                          <input autoFocus value={editVal} onChange={e=>setEditVal(e.target.value)} style={{width:70,padding:"3px 8px",border:`1px solid ${C.border}`,borderRadius:6,fontSize:13,textAlign:"right",background:C.bg,color:C.text}}/>
                        ...
                        <>
                          ...
                          <button onClick={e=>{e.stopPropagation();setEditingBudget(c.key);setEditVal(String(b));}} style={{fontSize:11,color:"#1a6bb5",border:"1px solid #1a6bb5",borderRadius:6,background:"none",padding:"2px 6px",cursor:"pointer"}}>Modifier</button>
                        </>
                      )}
                    </div>
                  </div>
                ...
                {isExp&&(
                  <div style={{borderTop:`0.5px solid ${C.border}`,background:C.bg2}}>
                    {catRes.length===0?<div style={{padding:"12px 14px",fontSize:13,color:C.text2,textAlign:"center"}}>Aucune dépense</div>:
                      catRes.map(r=>(
                        <div key={r.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderBottom:`0.5px solid ${C.border}`}}>
                          ...
                          <div style={{textAlign:"right"}}>
                            ...
                            {r.paymentStatus&&r.paymentStatus!=="Non payé"&&<div style={{fontSize:10,color:C.text2,marginTop:2}}>{r.paidBy||"Divisé"}</div>}
                          </div>
                        </div>
                      ))
                    }
        ...
        <>
          {persons.length===0?<div style={{textAlign:"center",padding:"24px 0",color:C.text2,fontSize:13}}>Aucune dépense</div>:
            persons.map(([name,total])=>{
              const diff=total-fairShare, pct=perPersonTotal>0?(total/perPersonTotal)*100:0;
                ...
                <div key={name} style={{border:`0.5px solid ${C.border}`,borderRadius:12,padding:"13px 14px",marginBottom:10,background:C.bg}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <div style={{width:34,height:34,borderRadius:"50%",background:"#e6f1fb",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#185fa5"}}>{name[0]?.toUpperCase()}</div>
                      <div><div style={{fontSize:14,fontWeight:600,color:C.text}}>{name}</div><div style={{fontSize:11,color:C.text2,marginTop:1}}>{pct.toFixed(0)}% des dépenses</div></div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontSize:18,fontWeight:700,color:"#1a6bb5"}}>{total.toFixed(0)} €</div>
                      <div style={{fontSize:11,color,marginTop:2,fontWeight:600}}>{diff>0?`+${diff.toFixed(0)} €`:diff<0?`${diff.toFixed(0)} €`:"Équilibré"}</div>
                    </div>
                  </div>
              ...
              {persons.map(([name,total])=>{
                ...
                return(
                  <div key={name} style={{fontSize:13,color:C.text,marginBottom:6,display:"flex",alignItems:"center",gap:6}}>
                    {diff<0?<><span style={{fontWeight:600,color:"#e24b4a"}}>{name}</span><span style={{color:C.text2}}>doit</span><span style={{fontWeight:600,color:"#1a6bb5"}}>{Math.abs(diff).toFixed(0)} €</span></>
