                ...
                <div style={{flex:1}}>
                  {dr.map(r=>(
                    <div key={r.id} style={{borderRadius:8,padding:"8px 10px",marginBottom:4,background:BG[r.type]||"#f0f0f0",cursor:"pointer"}} onClick={()=>onEdit(r)}>
                      ...
                      {(r.timeStart||( r.price&&(r.dateStart===d.iso||r.dateStart===r.dateEnd)))&&(
                      <div style={{fontSize:11,color:COLORS[r.type],opacity:0.8,marginTop:1}}>
                        {r.timeStart||""}{r.timeStart&&r.price&&(r.dateStart===d.iso||r.dateStart===r.dateEnd)?" · ":""}{r.price&&(r.dateStart===d.iso||r.dateStart===r.dateEnd)?`${r.price}€`:""}
                      </div>
                    )}
                    </div>
                  ))}
