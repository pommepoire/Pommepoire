...
export default function TripSelector({user,currentTripId,onSwitch,onClose,C}){
  ...
  useEffect(()=>{
    async function load(){const q=query(collection(db,"trips"),where("members","array-contains",user.uid));const snap=await getDocs(q);setTrips(snap.docs.map(d=>({id:d.id,...d.data()})));}
    load();
  },[user.uid]);
  ...
  async function createTrip(){
    ...
    await setDoc(newRef,{name:form.destination,destination:form.destination,dateStart:form.dateStart||"",dateEnd:form.dateEnd||"",code,members:[user.uid],memberNames:{[user.uid]:user.displayName||"Moi"},createdBy:user.uid,createdAt:serverTimestamp()});
    await updateDoc(doc(db,"users",user.uid),{tripId:newRef.id});
    onSwitch(newRef.id);setLoading(false);
  }
  async function saveEdit(){
    ...
    setLoading(true);
    await updateDoc(doc(db,"trips",editingTrip.id),{destination:editForm.destination,name:editForm.destination,dateStart:editForm.dateStart,dateEnd:editForm.dateEnd});
    setTrips(ts=>ts.map(t=>t.id===editingTrip.id?{...t,...editForm,name:editForm.destination}:t));
    setEditingTrip(null);setLoading(false);
  }
  return(
              ...
              {editingTrip?.id===t.id?(
                <div style={{padding:14,background:C.bg2}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    <div><div style={{fontSize:11,color:C.text2,marginBottom:3}}>Départ</div><input style={inp(C)} type="date" value={editForm.dateStart} onChange={e=>setEditForm(f=>({...f,dateStart:e.target.value}))}/></div>
                    <div><div style={{fontSize:11,color:C.text2,marginBottom:3}}>Retour</div><input style={inp(C)} type="date" value={editForm.dateEnd} min={editForm.dateStart} onChange={e=>setEditForm(f=>({...f,dateEnd:e.target.value}))}/></div>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:8}}>
                    <button onClick={saveEdit} disabled={loading} style={{padding:10,background:"#1a6bb5",border:"none",borderRadius:10,color:"white",fontSize:14,fontWeight:700,cursor:"pointer"}}>{loading?"…":"Enregistrer"}</button>
                ...
                <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",cursor:"pointer",background:t.id===currentTripId?"#f0f7ff":C.bg}} onClick={()=>onSwitch(t.id)}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <button onClick={e=>{e.stopPropagation();setEditingTrip(t);setEditForm({destination:t.destination||t.name||"",dateStart:t.dateStart||"",dateEnd:t.dateEnd||""});setError("");}} style={{fontSize:12,color:"#1a6bb5",border:"1px solid #1a6bb5",borderRadius:8,background:"none",padding:"4px 8px",cursor:"pointer"}}>Modifier</button>
                  </div>
                </div>
              )}
            </div>
          ))}
          {!creating?<button onClick={()=>setCreating(true)} style={{width:"100%",padding:12,border:`1.5px dashed ${C.border}`,borderRadius:12,background:"none",color:C.text2,fontSize:14,cursor:"pointer",marginTop:4}}>+ Créer un nouveau voyage</button>:(
